...
interface ApprovedListProps {
    approvedCourseList: Array<Course> | null;
    onDeleteStatus: (id: number) => Promise<void>;
}

function ApprovedList({ approvedCourseList, onDeleteStatus }: ApprovedListProps) {
    if (!approvedCourseList || approvedCourseList.length == 0) {
        return (
            <div>
                <div>강의가 없습니다.</div>
            </div>
    ...
    return (
        <div className="container">
            <div className="header-container">
                <p className="page-header-banner">승인된 강의 목록</p>
            </div>
            <br/>
            <div className="table-wrapper">
                <table className="modern-table">
                    ...
                    <tbody>
                        {approvedCourseList.map((item, index) => (
                            <tr key={ index }>
                                <td>{ index + 1 }</td>
                                ...
                                <td><button className="btn-base btn-danger" onClick={ () => { onDeleteStatus(item.courseId) } }>삭제하기</button></td>
                            </tr>
                        ))}
                    </tbody>
                </table>
